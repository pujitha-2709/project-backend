...
function convertMessagesToMistralMessages(messages) {
	...
	const getTools = (message) => {
		if (__langchain_core_messages.AIMessage.isInstance(message) && !!message.tool_calls?.length) return message.tool_calls.map((toolCall) => ({
			...toolCall,
	...
	return messages.flatMap((message) => {
		...
		else if (__langchain_core_messages.AIMessage.isInstance(message)) if (toolCalls === void 0) return [{
			role: getRole(message.getType()),
			content
		}];
		else {
